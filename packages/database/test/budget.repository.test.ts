...
describe('BudgetRepository', () => {
  it('returns the current global daily budget summary', async () => {
    ...
    expect(db.query).toHaveBeenCalledWith(expect.stringContaining("scope = 'global_daily'"), ['2026-08-27T00:00:00.000Z']);
  });
  ...
  it('rejects a reservation when either durable budget scope is exhausted', async () => {
    ...
    expect(client.query).not.toHaveBeenCalledWith(expect.stringContaining('INSERT INTO budget_reservations'), expect.anything());
  });
