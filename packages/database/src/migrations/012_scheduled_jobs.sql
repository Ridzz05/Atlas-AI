...
BEGIN
    ...
    IF EXISTS (
        ...
        WHERE table_name = 'scheduled_jobs' AND column_name = 'id' AND data_type = 'uuid'
    ) THEN
        ALTER TABLE scheduled_jobs ALTER COLUMN id TYPE VARCHAR(64) USING id::text;
    END IF;

    -- Add missing columns
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'scheduled_jobs' AND column_name = 'job_type') THEN
        ALTER TABLE scheduled_jobs ADD COLUMN job_type VARCHAR(64) NOT NULL DEFAULT 'cron';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'scheduled_jobs' AND column_name = 'cron_pattern') THEN
        ALTER TABLE scheduled_jobs ADD COLUMN cron_pattern VARCHAR(64);
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'scheduled_jobs' AND column_name = 'cron_expression') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'scheduled_jobs' AND column_name = 'timezone') THEN
        ALTER TABLE scheduled_jobs ADD COLUMN timezone VARCHAR(64) NOT NULL DEFAULT 'UTC';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'scheduled_jobs' AND column_name = 'assigned_agent') THEN
        ALTER TABLE scheduled_jobs ADD COLUMN assigned_agent VARCHAR(64);
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'scheduled_jobs' AND column_name = 'agent_id') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'scheduled_jobs' AND column_name = 'enabled') THEN
        ALTER TABLE scheduled_jobs ADD COLUMN enabled BOOLEAN;
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'scheduled_jobs' AND column_name = 'is_active') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'scheduled_jobs' AND column_name = 'created_by') THEN
        ALTER TABLE scheduled_jobs ADD COLUMN created_by VARCHAR(128) NOT NULL DEFAULT 'system';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'scheduled_jobs' AND column_name = 'last_error') THEN
    ...
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'scheduled_jobs' AND column_name = 'cron_expression' AND is_nullable = 'NO'
    ) THEN
    ...
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'scheduled_jobs' AND column_name = 'agent_id' AND is_nullable = 'NO'
    ) THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'scheduled_jobs_cron_nonempty') THEN
        ALTER TABLE scheduled_jobs ADD CONSTRAINT scheduled_jobs_cron_nonempty CHECK (length(trim(cron_pattern)) > 0);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'scheduled_jobs_payload_object') THEN
