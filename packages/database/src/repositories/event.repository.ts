...
export class SystemEventRepository {
  ...
  public async listAfterId(afterId: string, limit = 100): Promise<SystemEvent[]> {
    ...
    const cursor = await this.db.query('SELECT event_id, occurred_at FROM event_outbox WHERE event_id = $1', [afterId]);
    ...
    const result = await this.db.query(
      ...
      LIMIT $3`,
      [cursorRow.occurred_at, cursorRow.event_id, normalizedLimit]
    );
