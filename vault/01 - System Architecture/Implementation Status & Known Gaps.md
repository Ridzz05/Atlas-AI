...
Masih terbuka dari sweep yang sama: `review.requiredAgent` dideklarasikan tetapi tidak ada tahap
yang menegakkannya (kini muncul di verdict QA), dan `IdempotencyRepository.expireStale` tidak
pernah dipanggil (kini hanya higiene — `claim` menyembuhkan dirinya sendiri).
...
| **Guard migrasi 012 tidak schema-qualified** — `CREATE TABLE IF NOT EXISTS` menulis ke schema pertama di `search_path`, sedangkan `information_schema.columns WHERE table_name = …` membaca tabel dari schema **mana pun**. Ketika keduanya berbeda, guard melewatkan `ALTER` yang diperlukan dan statement berikutnya gagal | `packages/database/src/migrations/012_scheduled_jobs.sql` | ✅ diperbaiki: `table_schema = current_schema()` di semua 13 lookup; `pg_constraint` lewat join `pg_class`/`pg_namespace` + filter `nspname`. Inilah yang membuat rantai migrasi bisa dijalankan di schema uji — dan karenanya bisa diuji |
| **Tidak ada test yang pernah menjalankan migrasi** — 65 test `packages/database` mem-mock pool dan memeriksa teks SQL, sehingga rantai migrasi tidak pernah dieksekusi | `scripts/verify-migrations.mjs` (baru), `packages/database/test/migration-schema-scope.test.ts` (baru) | ✅ diperbaiki: `pnpm db:verify` menerapkan seluruh rantai ke PostgreSQL sungguhan di schema scratch (11/11 check lolos: 17 migrasi, tabel, kolom fase 016, indeks `occurred_at`, `gen_random_uuid()`, seeding 9 agen, riwayat imutabel, 8 tool terdaftar) + kontrak statis atas semua berkas migrasi. `ci-compose-smoke.sh` kini juga menuntut jumlah migrasi terpasang = jumlah berkas |
| `memory.propose_write` selalu melaporkan `duplicate: false`, dan bila yang cocok adalah item `verified` respons yang sama membawa `status: 'verified'` — terbaca sebagai "fakta Anda sudah kanonik" | `packages/memory/src/pipeline/proposal-service.ts`, `tools/memory-tools.ts` | ✅ diperbaiki: `propose()` mengembalikan `{ item, duplicate }` (hanya ia yang tahu mana yang terjadi); tool meneruskannya |
| Dedup proposal mencocokkan item **semua status** — proposal yang isinya pernah di-deprecate mengembalikan record yang sudah ditolak, sehingga proposal hilang tanpa jejak | `packages/memory/src/pipeline/proposal-service.ts` | ✅ diperbaiki: hanya item hidup (`unverified`/`verified`) yang dianggap duplikat; deprecation adalah keputusan atas satu record, bukan atas konten selamanya |
| **Tidak ada jalur promosi ke `verified`** — agen menulis `unverified`, bacaan agen verified-only, dan `verify()`/`deprecate()` tidak dipanggil apa pun. Seluruh subsistem memori inert; satu-satunya cara mengubah status adalah mengedit DB langsung, melewati audit trail | `apps/agent-service/src/routes/metadata.ts`, `apps/dashboard/src/app/brain/page.tsx` | ✅ diperbaiki: `POST /api/v1/memory/:id/verify` + `/:id/deprecate` di belakang bearer token operator (principal yang sama yang memutuskan approval); panel memori dashboard kini menampilkan status + tombol aksi |
| `updateStatus` tidak memfilter kedaluwarsa, jadi mempromosikan item kedaluwarsa melaporkan sukses padahal semua jalur baca memfilternya — item tetap tak terlihat selamanya | `packages/memory/src/pipeline/proposal-service.ts` | ✅ diperbaiki: `verify`/`deprecate` menolak item yang tidak bisa dibaca balik (404 + pesan menyebut kedaluwarsa) |
| **Tool terdaftar yang tidak bisa dipanggil agen mana pun adalah deklarasi inert** — registry gagal-tertutup pada `allowedTools`, jadi keterjangkauan adalah properti definisi agen | `apps/worker/test/tool-reachability.test.ts` (baru) | ✅ tercatat: `communication.send_approved` (chief & hermes mendeklarasikannya di `humanApprovalFor` tetapi tidak di `tools`, jadi alur approval-nya mustahil dimasuki) dan 6 entri allowlist tanpa implementasi (`tasks.create_child`, `tasks.update_status`, `tasks.get`, `tasks.list`, `approvals.request`, `brand.get_voice` — dua di antaranya juga ada di `NO_APPROVAL_ACTIONS`, yang meng-auto-approve aksi yang tidak ada). Keduanya **keputusan izin/kapabilitas**, bukan bug dengan satu jawaban benar — dicatat sebagai inventaris yang direview; permukaan tool yang terjangkau juga dipin |
| **Gerbang intake gagal-terbuka** — `createIntakeGate` dimulai dengan `if (!options.controlStateRepo) return;`, jadi pemanggil yang tidak menyediakan sumber kontrol mendapat intake yang berjalan seolah operator tidak pernah menekan pause | `apps/agent-service/src/routes/intake-gate.ts` | ✅ diperbaiki: menjawab 503 bila sumber kontrol tidak ada (konsisten dengan route kontrol). 8 test lama harus menyatakan status sistem lewat `test/support/control-state.ts` — memang itu intinya |
| **Tidak ada verifikasi API terhadap server + database sungguhan** — 84 test injected mem-mock repository, jadi pengkabelan route↔repository tidak pernah dijalankan | `scripts/verify-api-live.mjs` (baru), `pnpm api:verify` | ✅ diperbaiki: boot server Fastify asli dengan repository `createAtlasRuntime`, inject request nyata, periksa efeknya di database — di dalam scratch schema (17/17 check). Inilah yang menemukan gerbang intake tanpa sumber kontrol: `buildServer` memakai nama `controlStateRepo` sedangkan runtime menyediakan `telegramStateRepo` |
| **`TaskPlanner.plan()` gagal-terbuka** — parsing, validasi schema, dan `PlanValidator.assertValid` ada dalam satu `try`, dan `catch` mengembalikan `generateFallbackPlan()`. Rencana yang **ditolak** (siklus dependensi, agen tak dikenal, id duplikat, >8 langkah, biaya di atas plafon) dibuang dan rencana kalengan dijalankan — penolakan itu tidak berpengaruh pada apa yang dieksekusi. Melanggar `docs/AGENTS.md:8` ("Fail Closed") | `packages/orchestration/src/planner/task-planner.ts` | ✅ diperbaiki: dua mode kegagalan dipisah — output tak terbaca tetap pakai fallback (dan tetap diumumkan ke operator), penolakan validator **menghentikan task**. Fallback kini juga divalidasi. Tidak ada test untuk TaskPlanner sama sekali; `test/task-planner.test.ts` (baru) menutup 7 kasus |

### 10.4 Sweep lanjutan kedua (21 Sep 2026) — sudah diperbaiki

Dua kelas bug yang muncul di gelombang ini, keduanya sudah dikenal tetapi belum pernah ditutup rapat:

1. **Satu aturan, banyak rute.** Aturan yang sama ditulis ulang di beberapa tempat dengan jawaban
   berbeda-beda, jadi rute yang paling jarang diuji adalah rute yang paling longgar.
2. **Kontrol yang tidak di-wire bukan kontrol.** Guard-nya ada dan benar, tetapi tidak ada pemanggil
   yang menyediakan dependensinya — jadi ia tidak pernah menyala.

| Butir | Anchor | Status |
| :--- | :--- | :--- |
| **Aturan scope Second Brain gagal-terbuka di tiga jalur.** `search` menyusun `readableScopes` hanya bila `allowedScopes` tidak kosong, dan `null` berarti **tanpa filter**; `listDocuments` punya bentuk yang sama; `getDocument`/`getDocumentByPath` tidak menerima scope sama sekali | `packages/memory/src/second-brain/second-brain-retriever.ts`, `second-brain-service.ts` | ✅ diperbaiki: aturan punya satu pemilik di `vault-root.ts` (`ScopeGrant`, `agentScopeGrant`, `isScopeReadable`, `filterByScopeGrant`, `GLOBAL_ONLY_GRANT`). `dataScopes` default `[]`, jadi "tanpa grant" kini berarti **global saja** untuk agen. Permukaan operator (`/api/v1/brain/*`) adalah principal yang berbeda dan kini **menyatakan** dirinya (`{ kind: 'operator' }`) alih-alih mewarisi lubang |
| `second_brain.read_note` mengabaikan konteksnya sepenuhnya — enam agen mencantumkannya, dan `search` menolak catatan yang sama | `packages/tools/src/tools/second-brain-tools.ts` | ✅ diperbaiki: grant diteruskan dari `ctx`; `test/second-brain-scope.test.ts` (baru) + 3 test tingkat tool |
| `second_brain.sync_vault` meneruskan scope dari model tanpa memeriksanya terhadap grant — agen bisa mengindeks isi vault ke scope yang tidak dimilikinya | `packages/tools/src/tools/second-brain-tools.ts` | ✅ diperbaiki: menolak scope di luar grant agen (sebelum guard path — ini pernyataan tentang pemanggil, bukan tentang lingkungan), selaras dengan `memory.propose_write` |
| `IngestVaultResult.skipped` tidak pernah dinaikkan — satu-satunya jalur yang melewati berkas (`contentHash`) tidak bisa memberi tahu pemanggilnya, jadi setiap laporan berbunyi "Files Skipped: 0" dan `totalFiles ≠ ingested + skipped + errors` | `packages/memory/src/second-brain/vault-ingestion-service.ts` | ✅ diperbaiki: `ingestDocument` mengembalikan `{ document, outcome }` (`created`/`updated`/`unchanged`/`rescoped`); walk menghitung `unchanged` sebagai skipped; route melaporkan `outcome` alih-alih menyiratkan tulisan |
| **Dua path berbeda berbagi satu id dokumen.** Slug menurunkan `notes/static.md` dan `notes-static.md` menjadi `notes-static-md`; ingest kedua menghapus chunk yang pertama dan menimpanya — catatan hilang dari indeks padahal berkasnya ada, dan sitasi mengembalikan isi satu catatan dengan judul catatan lain | `packages/memory/src/second-brain/vault-ingestion-service.ts` | ✅ diperbaiki: id = slug + digest pendek dari path persis (stabil antar-run, tidak lagi ambigu) |
| Ingest ulang konten yang tidak berubah mengembalikan dokumen tersimpan **sebelum** scope dihitung ulang, jadi tidak ada jalur tulis yang bisa memindahkan catatan ke scope lebih sempit — semuanya melaporkan sukses | `packages/memory/src/second-brain/vault-ingestion-service.ts` | ✅ diperbaiki: perubahan scope tidak butuh re-embedding (vektor berasal dari konten), jadi diterapkan di tempat pada dokumen **dan** chunk-nya; outcome `rescoped` |
| **Satu pertanyaan — "apakah run ini masih hidup?" — dijawab tiga predikat berbeda.** `recoverStaleRuns` menganggap `waiting_approval` hidup, lintasan task-nya juga, sedangkan `recoverStaleReservations` menganggapnya mati begitu lease-nya lewat | `packages/database/src/repositories/run-liveness.ts` (baru), `run.repository.ts`, `budget.repository.ts` | ✅ diperbaiki: predikat punya satu pemilik (`runIsAliveSql`, `runIsRecoverableSql`, dibangun untuk alias yang dipakai tiap pemanggil). Run yang parkir menunggu manusia hidup tepat selama `approvals.expires_at` belum lewat — lease tidak bisa menjawabnya karena run `waiting_approval` memang tidak memegang lease. Akibat sebelumnya: run dan task tersangkut selamanya sementara reservasi anggarannya dilepas di bawahnya |
| **`reserve()` tidak idempoten per run.** Jalur resume approval memanggil `reserve()` kedua kalinya untuk `runId` yang sama; dua baris hidup untuk satu run, hanya id terbaru yang di-settle, `reserved_usd` membengkak satu plafon per-run selama ≤15 menit, dan `reserve()` mengembalikan null untuk run lain → `BUDGET_EXCEEDED` padahal belum ada yang dibelanjakan | `packages/database/src/repositories/budget.repository.ts` | ✅ diperbaiki: `reserve()` mencari reservasi hidup milik run lebih dulu, mengembalikannya bila sudah menutupi permintaan, dan hanya menambah selisihnya. Satu run, satu reservasi |
| **`/stop` melaporkan sukses yang tidak dimilikinya** — `rowCount` dibuang, error `updateStatus` ditelan, balasan selalu "cancellation signal sent", termasuk saat tidak ada yang cocok dan saat task sudah selesai (yang dilabeli ulang `cancelled` dan menimpa `completed_at`) | `apps/telegram-bot/src/handlers/commands.ts` | ✅ diperbaiki: membaca task lebih dulu, menolak task terminal, melaporkan nol run yang disinyalir secara jujur, dan menampilkan kegagalan `updateStatus` alih-alih menyembunyikannya |
| **`/pause` menjatuhkan aktor** — `commands.ts` mengirim `actorId`, callback di `bot.ts` ditulis satu parameter, jadi `updated_by` ditulis dengan default `telegram-owner` | `apps/telegram-bot/src/bot.ts` | ✅ diperbaiki: aktor diteruskan; alasan pembatalan kini menyebut aktor (`/stop` sebelumnya tidak mengirim aktor sama sekali) |
| **`/emergency_stop` menjanjikan tiga hal yang tidak dilakukannya** — "semua run aktif telah dikirim abort signal" (padahal tanpa runner/runRepo tidak ada yang disinyalir) dan "external writes are locked" (tidak ada apa pun di `packages/tools` atau `packages/orchestration` yang membaca status kontrol; pembatalan bersifat kooperatif dan dipoll antar-turn) | `apps/telegram-bot/src/handlers/commands.ts` | ✅ diperbaiki: balasan disusun dari apa yang benar-benar terjadi, dan menyatakannya bila tidak ada signaller yang terpasang |
| **Gerbang intake melewatkan satu rute POST** — `POST /api/v1/automations/scheduled-jobs` membuat job terjadwal durabel yang dinyalakan scheduler worker tanpa membaca status kontrol, jadi emergency stop tetap mengakumulasi pekerjaan | `apps/agent-service/src/routes/automations.ts` | ✅ diperbaiki: `preHandler: intakeGate`. Penyebabnya adalah **test**-nya: `intake-gate.test.ts` menelusuri daftar tulis tangan berisi lima rute, jadi ia tidak bisa melihat rute keenam |
| **Test yang cakupannya sebuah daftar tidak bisa melaporkan apa yang hilang dari daftar** | `apps/agent-service/test/intake-gate-inventory.test.ts` (baru) | ✅ diperbaiki: membaca **sumber** rute; setiap `app.post(...)` di bawah `src` harus digerbangi atau masuk `EXEMPT` beserta alasannya. Reader-nya juga mengizinkan bentuk `app.post<{ Params: … }>` — versi pertamanya hanya mengenali satu bentuk dan diam-diam melewatkan dua rute di `metadata.ts` |
| **Feed komunikasi membeku di 100 baris tertua** — `list()` selalu `ORDER BY created_at ASC … LIMIT n`; dashboard meminta `limit=100`, menerima 100 pesan pertama yang pernah ditulis, mengurutkannya terbaru-dulu, dan mengambil halaman yang sama lagi di setiap event stream | `packages/database/src/repositories/message.repository.ts`, `tool-call.repository.ts`, `apps/dashboard/src/app/communications/page.tsx` | ✅ diperbaiki: arah jendela jadi argumen eksplisit (`order: 'oldest' | 'newest'`, default kronologis untuk thread); route memvalidasinya dan menolak nilai tak dikenal; feed meminta `newest` |
| **Stream SSE tidak pernah pulih** — setiap halaman membuat satu `EventSource` di dalam `useEffect` dan tidak pernah membuatnya lagi; proxy API menjawab 503 untuk jalur stream saat API tak terjangkau, dan menurut spesifikasi EventSource respons non-200 menggagalkan koneksi **secara permanen** | `apps/dashboard/src/lib/event-stream.ts`, `test/event-stream.test.ts` (baru) | ✅ diperbaiki: `createAtlasEventStream` memiliki siklus hidupnya (backoff berbatas, listener lama dilepas lebih dulu agar tidak dobel, backoff direset setelah berhasil) dan melaporkan `AtlasStreamStatus` nyata yang diterjemahkan `streamStatusLabel`. Halaman tidak lagi mencetak "reconnecting" di atas koneksi yang tidak akan kembali — dua halaman yang menampilkannya hanya membalik boolean tanpa mencoba reconnect |
| **Tiga pembacaan daftar menyortir kolom tanpa indeks** — `messages` dan `tool_calls` (dua sumber feed) serta `audit_events` (pembacaan default `GET /api/v1/audit`, tidak pernah dipangkas): setiap bacaan adalah sequential scan plus sort | `packages/database/src/migrations/018_list_read_indexes.sql` (baru) | ✅ diperbaiki: `(created_at DESC, id DESC)` dan `(timestamp DESC, id DESC)`; `id` sebagai tiebreaker agar batas jendela deterministik saat beberapa baris berbagi milidetik. `pnpm db:verify` kini menuntut indeksnya benar-benar ada di server sungguhan (**14/14**) dan jumlah migrasi diturunkan dari direktori, bukan plafon hardcode yang berhenti menyadari migrasi yang tak pernah diterapkan |
