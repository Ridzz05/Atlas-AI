...
> Release status (26 August 2026): local lint, typecheck, tests, and production build pass. Authenticated event replay, durable global/per-run budget accounting, worker lease recovery, aggregate cost/budget metrics, and read-only governance settings are implemented; real PostgreSQL/Redis boot, backup/restore, and restart recovery still require a Docker environment. External writes remain disabled until approved connectors and providers are verified.
