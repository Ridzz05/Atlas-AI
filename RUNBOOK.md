...
History persistence is best-effort around task creation so a history outage does not cause a client retry to create duplicate tasks. The service logs the failed history write; monitor that log and repair the database before treating the communication feed as complete.
