...
CI runs `pnpm audit --audit-level high` after `pnpm install --frozen-lockfile`. The current lockfile resolves patched dashboard dependencies (`next@15.5.24`, `sharp@0.35.3`, and `postcss@8.5.26`); the latest audit found zero high/critical vulnerabilities, and `pnpm audit signatures` verified 331 packages. Do not use forced audit remediation; review and test each dependency update with its lockfile diff.
