    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-400">Verified and proposed memory records from the durable store.</p>
        </div>
        <button onClick={() => void load()} className="p-2 rounded-lg text-gray-400 hover:bg-gray-800" aria-label="Refresh memory">
      ...
      <div className="bg-[#111827] border border-gray-800 rounded-xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-10 text-center text-xs text-gray-400">
            ...
            No memory items have been recorded yet.
          </div>
        ) : (
            ...
            {items.map(item => (
              <div key={item.id} className="p-4">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span className="text-[10px] uppercase text-gray-400">{item.status}</span>
                </div>
                <p className="mt-2 text-xs text-gray-300">{item.content}</p>
