...
interface AgentListResponse { data: ApiAgent[]; }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex items-start justify-between gap-4">
        ...
        <button onClick={() => void loadAgents()} className="p-2 rounded-lg text-gray-400 hover:bg-gray-800" aria-label="Refresh agent registry">
      ...
      {loading ? <div className="p-12 text-center text-xs text-gray-400" role="status" aria-busy="true">Loading live agent registry…</div> : agents.length === 0 ? (
        <div className="p-12 text-center bg-[#111827] border border-gray-800 rounded-xl text-xs text-gray-400">No agents are registered in the control plane.</div>
      ) : (
          ...
          {agents.map(agent => (
            <div key={agent.id} className="p-6 bg-[#111827] border border-gray-800 rounded-xl space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-gray-800">
                ...
                <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">REGISTERED</span>
              </div>
              <p className="text-xs text-gray-300">{agent.description}</p>
              <div className="space-y-2 text-xs">
                <div className="flex items-center gap-1.5 text-gray-400 font-mono text-[11px]"><Wrench className="w-3.5 h-3.5" /><span>ALLOWED TOOLS ({agent.permissions.tools.length})</span></div>
                <div className="flex flex-wrap gap-1.5">
                  {agent.permissions.tools.length === 0 ? <span className="text-[10px] text-gray-500">No tool permissions</span> : agent.permissions.tools.map(tool => <span key={tool} className="px-2 py-0.5 rounded bg-gray-800/80 text-gray-300 font-mono text-[10px] border border-gray-700/60">{tool}</span>)}
                </div>
              </div>
