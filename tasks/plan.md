...
- Git history now includes the implementation slices through `cd73fbe`; the original `a6efa4f` “complete” commit was a scaffold checkpoint, not a production proof.
...
2. Add prompt-injection-safe fetched-content handling and size/network limits. The tool boundary now rejects credential-bearing URLs, literal IPv4/IPv6 targets, and local/internal hostnames; an approved provider must still enforce DNS, redirect, response-size, and timeout controls for its actual network request.
