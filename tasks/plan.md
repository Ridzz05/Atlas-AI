...
- Git history now includes the implementation slices through `98ea6f6`; the original `a6efa4f` “complete” commit was a scaffold checkpoint, not a production proof.
...
4. **Research is implemented but deliberately opt-in and not live-verified here.** `98ea6f6` adds a Brave adapter with source, freshness, confidence, sensitivity, unresolved-question evidence, public-DNS/redirect/size/timeout controls, and an untrusted-content boundary. A clean environment with an approved API key must still verify provider behavior and operations.
...
2. Add prompt-injection-safe fetched-content handling and size/network limits. The tool boundary and `SafeWebFetcher` now reject credential-bearing/local targets, validate public DNS and redirects, bound response size, and enforce timeouts; gateway cancellation propagates an `AbortSignal`. DNS pinning/rebinding protection for the actual outbound socket remains a provider release-review item.
...
Historical execution through `76837ea` covered repository formatting enforcement, dependency/signature audit gates, timeout cancellation, fail-closed external writes, rubric governance, verified-only memory, and the durable control plane. The current research slice is committed in `98ea6f6`: it adds the opt-in Brave adapter, bounded safe fetch, runtime/worker wiring, strict research environment validation, and provider-to-Tool-Gateway tests. The latest full local code gate passed: `pnpm.cmd format:check`, `pnpm.cmd lint`, `pnpm.cmd typecheck` (26/26), `pnpm.cmd test -- --reporter=dot` (26/26), and `pnpm.cmd build` (15/15). The tools suite passes 40 tests and the runtime suite passes 5 tests. The previously recorded dependency audit remains zero high/critical with 332/332 registry signatures verified. Docker is unavailable in this environment, so a successful PostgreSQL/Redis Compose boot, budget/lease behavior against real PostgreSQL, backup restore, restart recovery, and live research verification remain unverified.
