...
- Git history now includes the implementation slices through `4121b3d`; the original `a6efa4f` “complete” commit was a scaffold checkpoint, not a production proof.
...
Remaining release blockers: no approved outbound connector, no verified real research adapters (the tool/provider boundaries now exist), no successful Docker boot/recovery/backup run yet, alerting/retention/rollback, and a configured per-agent aggregate budget policy if required by the owner.
