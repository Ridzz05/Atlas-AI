...
- Git history now includes the implementation slices through `bcdb1d4`; the original `a6efa4f` “complete” commit was a scaffold checkpoint, not a production proof.
...
5. **Dashboard governance is only partially mutable.** Authenticated event SSE with replay, read-only artifact/memory/audit APIs, durable aggregate cost/budget metrics, environment-backed settings, persisted active-rubric loading, and durable pause/resume/emergency-stop actions are exposed. Other mutable settings, including operator-facing rubric version changes, remain deployment/API-controlled by design.
...
4. Persist a configurable rubric repository with immutable version history and validate all score dimensions/evidence before artifact export. The repository and active-version runtime hydration are implemented; an authenticated operator-facing mutation API remains a follow-up.
