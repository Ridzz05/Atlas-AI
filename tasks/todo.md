...
Current release state supersedes the historical implementation inventory below: the latest implementation checkpoint is `784394e`, with task-status integrity, lifecycle events, API/Telegram intake message persistence, durable dashboard Tasks/Communications feeds, live SSE refresh, prefix-isolated Compose containers, durable intake coverage in the Compose smoke test, backup/restore prefix compatibility, and duplicate-safe queued-task recovery. The focused slice passes dashboard 9/9, agent-service 8/8, Telegram 17/17, orchestration 24/24, database 3/3, and queue/worker recovery 18/18 tests. The full local gates pass format, lint, typecheck 26/26, test 26/26, and build 15/15; Docker-backed recovery and live provider/connector validation remain release criteria.
...
Commit `784394e` prevents periodic queued-task recovery from racing with an active/deferred BullMQ job, removes terminal queue records before requeueing a still-queued task, and applies task-level deduplication to the in-memory queue. Its focused queue/worker regression suite passes 18/18.
...
The historical inventory below is retained for audit context; its gate counts predate the current DNS-pinned research slice and must not be used as the current release status.
